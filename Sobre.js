export default function sobre(){
    <head>
        <link rel="stylesheet" 
                href="globals.css">
    </head>
        <body display: flex; style="background-color:#603b88">
            <h2 class=sobrenos style="color: white;";>
                 SOBRE NOS
            </h2>
        <div class="sobre">
            <div class=texto style="background-color:#242424;height:180;width:40;">
                    <p class=letras style="color: white";>
                        A IDE é a empresa júnior dos cursos Ciência e
                    Engenharia de Computação da Universidade 
                    Federal do Rio Grande do Sul. Formado por alunos 
                    interessados em se tornarem empreendedores a 
                    profissionais excepcionais, a IDE fornece soluções 
                    em software, como sites e sistemas, com preços 
                    abaixo do mercado, aliados à excelência da 
                    UFRGS, o Instituto de Informática e seus 
                    professores, que fornecem apoio técnico e 
                    aconselhamento aos estudantes.</p>
                    
                    <img class=ide src="logo-pixelado-branco.png"height="30">
                   
            </div>
            <img class=neko src="neko-comemorando.png" height="500">
        </div>
             
    </body>
 }